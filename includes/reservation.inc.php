...
if (isset($_POST['reserv-submit'])) {
    ...
    if (
        ...
        !preg_match("/^[a-zA-Z 0-9]*$/", $comments) ||
        !between($comment, 0, 200)
    ) {
        ...
        if ($current_tables + $tables > $a_tables) {
            ...
            $sql =
                "INSERT INTO reservation(f_name, l_name, num_guests, num_tables, rdate, time_zone, telephone, comment, user_fk) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
